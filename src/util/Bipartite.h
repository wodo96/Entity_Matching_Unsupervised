...
class Bipartite
{
	public:
			...
			for(int iter=0;iter < 50;iter++){
				...
				for(int wid=0;wid < word_num;wid++){
					...
					if(edges[new_wid].size() > 0){
						...
						//score = pow(2.718, 1/(1+1/score));
						score = scale*1/(1+1/(score));
				...
				//cout<<"zzzzzzzz: "<<t_score[277]<<endl;
		 ...
		 *
		 */
		void updatePScore(double* p_conf)
		{
			for(int iter=0;iter < 50;iter++){
				//cout<<"iteration "<<iter<<endl;
				//cout<<t_score[1]<<"\t"<<t_score[2]<<endl;
				...
				for(int wid=0;wid < word_num;wid++){
					...
					if(edges[new_wid].size() > 0){
						...
						//score = scale*1/(1+pow(2.718,-(score-0.5)));
